...
const { getRepositoryData } = require('../settings');
...
const onRead = async req => {
  ...
  const { results } = await srv
    .cmisQuery(repository.id, query, options)
    .execute(destination);
...
const onReadStream = async (req, cmisDocument) => {
  ...
  const content = await srv
    .downloadFile(repository.id, cmisDocument['cmis:objectId'], {
      download: 'inline',
      config: {
...
const onCreate = async req => {
  ...
  const result = await srv
    .createDocument(repository.id, cmisDocument['cmis:name'], null, {
...
const onCreateFolder = async folderName => {
  ...
  return await srv.createFolder(repository.id, folderName).execute(destination);
};
...
const onUpdateContentStream = async req => {
  ...
  let document = await srv
    .getObject(repository.id, cmisDocument['cmis:objectId'])
    .execute(destination);
  ...
  document = await srv
    .appendContentStream(repository.id, documentId, filename, data.content)
    .execute(destination);
...
const onDelete = async req => {
  ...
  const srv = await cds.connect.to('cmis-client');
  await srv
    .deleteObject(repository.id, cmisDocument['cmis:objectId'])
    .execute(destination);
};
