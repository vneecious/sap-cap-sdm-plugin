...
const onRead = async req => {
  ...
  let folderName;
  try {
    folderName = deriveFolderNameFromNavigation(req);
  } catch (e) {
    // nothing to do
  }
...
const _isStreaming = req => {
  const mediaPropertyName = Object.entries(req.target.elements).find(
    ([, value]) => value['@Core.MediaType'],
  )?.[0];

  if (!mediaPropertyName) return false;

  return Boolean(
    req?.query?.SELECT?.columns?.find(c =>
      c?.ref?.find(r => r == mediaPropertyName),
    ),
  );
};
